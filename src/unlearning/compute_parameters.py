...
def compute_sigma(c_0, c_1, gamma_t, lambda_t, epsilon, q):
    B = 2 * q / epsilon
    t = lambda_t * c_0 / c_1
    sigma = (B * gamma_t * (2 - gamma_t * lambda_t) * c_0 * c_1 * (2 - t)) ** 0.5
    return sigma
...
def stable_roots_from_params(beta0, beta1, gamma):
    ...
    TODO: Implement numerically stable root computation.
    """
    raise NotImplementedError("Stable root computation not implemented yet.")
    ...
    for block in configs:
        ...
        config["sigma_new"] = (
            calculate_noise_coefficient(
                config["gamma_t"], config["lambda_t"], config["M"]
            )
            * config["sigma"]
        )
